...
Когда нужно видеть всё разом относительно последнего коммита — `<leader>gB` переключает базу по кругу: **индекс → HEAD → HEAD~1**. При базе, отличной от индекса, git не даёт применять и откатывать ханки, поэтому база по умолчанию именно индекс. Если после `<leader>gB` перестали работать `<leader>gs` и `<leader>gr` — конфиг об этом скажет; верните базу на «индекс» тем же `<leader>gB`.

### Если git-команда не сработала

Команды `<leader>g*`, работающие с ханками, требуют, чтобы курсор был **в окне файла** — в проводнике, в пустом буфере или в служебном окне ханков просто нет. В таких случаях конфиг говорит, что именно не так, всплывающим уведомлением, а не молчит.

Не требуют файла и работают откуда угодно: `<leader>gD` (панель changeset'а), `<leader>gm` (дифф ветки), `<leader>gF` (история репозитория), `<leader>gg` (lazygit).
...
Если при запуске появляется сообщение blink.cmp про `build`, значит не собралась его Rust-библиотека нечёткого поиска. Лечится одной командой:

```vim
:Lazy build blink.cmp
```

Для сборки нужен Rust toolchain (`rustup`).
...
| `<leader>ut` | Выбрать тему (живой предпросмотр) |
| `<leader>ug` | Отступные линии |
...
| `<leader>e` | Перейти в проводник, повторное нажатие — вернуться в файл |
| `<leader>E` | Показать текущий файл в проводнике |
| `q` внутри проводника | Закрыть проводник |
...
| [fidget.nvim](https://github.com/j-hui/fidget.nvim) | Всплывающие уведомления и прогресс индексации LSP |
| [snacks.nvim](https://github.com/folke/snacks.nvim) | Стартовый экран, отступные линии, защита от больших файлов |
| [dropbar.nvim](https://github.com/Bekaboo/dropbar.nvim) | Хлебные крошки: путь и символ под курсором |
| [catppuccin](https://github.com/catppuccin/nvim), [tokyonight](https://github.com/folke/tokyonight.nvim), [kanagawa](https://github.com/rebelot/kanagawa.nvim), [rose-pine](https://github.com/rose-pine/neovim) | Дополнительные темы |
...
**Статуслайн** — один на всё окно (`laststatus = 3`), а не по одному на каждое: раньше рядом со статуслайном файла висел ещё один, от проводника. Показывает режим, файл (длинные пути ужимаются), git-ветку и счётчик правок `+N ~N -N`, активные LSP-серверы, диагностику по уровням, позицию курсора и тип файла. Цвета берутся из активной темы, поэтому переключение темы перекрашивает и его.

**Панель вкладок** — активный буфер выделен полоской и фоном, на вкладке видны значок несохранённых изменений и счётчик ошибок. Левый клик переключает, средний закрывает. Скрывается при одном открытом буфере. Над проводником — имя проекта.

**Хлебные крошки** над файлом: `src › utils › parse.ts › parseConfig`. Путь плюс символ под курсором, как в VS Code.

**Отступные линии** с подсветкой текущего блока — структура сгенерированного агентом кода видна сразу.

**Стартовый экран** при запуске без файла: недавние файлы, поиск, дифф изменений. Проводник в этот момент не открывается, чтобы не зажимать экран, — он появляется, как только открыт первый файл.

Все плавающие окна с единой скруглённой рамкой (`winborder`).

### Темы

`<leader>ut` открывает список тем с живым предпросмотром: выбранная применяется сразу и запоминается между сессиями (пишется в `stdpath("state")/colorscheme`).

Предустановлены cyberdream (по умолчанию), catppuccin, tokyonight, kanagawa и rose-pine — все с прозрачным фоном, чтобы вид не менялся скачком при переключении.
...
lua/colorscheme.lua         выбор темы и её сохранение между сессиями
