...
|-----|---------|----------------|
| 1. Посмотреть весь changeset | `<leader>gD` | Панель со списком всех изменённых файлов слева, side-by-side дифф справа. `<Tab>` — следующий файл. Удалённые файлы — красные и зачёркнутые |
| 2. Дифф конкретного файла | `<leader>gd` | Вертикальный сплит: слева версия из HEAD («было»), справа текущая («стало»). Изменения подсвечены построчно и посимвольно, прокрутка связана |
...
В любом диффе — и в сплите `<leader>gd`, и в панели `<leader>gD` — половины прокручиваются вместе, по вертикали и по горизонтали, и одинаковые строки всегда стоят друг напротив друга. Над каждой половиной подпись: «было · HEAD» (или «индекс» для `<leader>gi`) и «стало».

Хлебные крошки в окнах диффа не показываются: они были бы только над рабочей копией, у версии из git их нет, и половины разъезжались бы на одну строку. После закрытия диффа крошки возвращаются.
...
`<leader>ut` открывает список тем с живым предпросмотром: выбранная применяется сразу и запоминается между сессиями вместе с вариантом — `rose-pine-moon` после перезапуска остаётся moon (пишется в `stdpath("state")/colorscheme`). Если сохранённая тема по какой-то причине не загрузилась, включается cyberdream, но твой выбор в файле не затирается.
